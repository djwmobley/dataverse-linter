...
| R37 | regex-template (`joinPlusContent` view) | ERROR | OData `$filter` equality or `$select` on a Lookup attribute uses the bare logicalname (e.g., `appmoduleidunique eq <guid>` in `$filter`, or `$select=...,appmoduleidunique` against `appmodulecomponents`) instead of the required `_<logicalname>_value` form. The bare logicalname is a navigation property for `$expand` only; using it in `$filter` or `$select` produces `0x80060888 Could not find a property named '<name>'`. v0.4.4 widening adds `$select` detection (Gap 1) anchored to the `appmodulecomponents` EntitySet, and detects patterns that straddle PowerShell `+`-concat line boundaries via the `joinPlusContent` content view (Gap 2). Implementation: hard-coded list of known Lookup logicalnames (seeded: `appmoduleidunique`); extend via `node src/rule-manager.js set-variables R37 <name1> ...`. Substrate: (1) https://learn.microsoft.com/en-us/power-apps/developer/data-platform/reference/entities/appmodulecomponent — `appmoduleidunique` Type=Lookup, Targets=appmodule; (2) https://learn.microsoft.com/en-us/power-apps/developer/data-platform/webapi/query/filter-rows — Lookup `$filter` pattern; (3) https://learn.microsoft.com/en-us/power-apps/developer/data-platform/webapi/query/select-columns — Lookup `$select` pattern (use `_<logicalname>_value`). |
...
**v0.4.1 — block-comment guard fix (round-2 of PR #3 review):** The guard view changed from `rawContent` to `rawContentNoBlockComments`. Round-2 found a false negative: a `#Requires -Version 5.1` lexeme nested inside a `<# ... #>` block comment satisfied the guard regex when tested against `rawContent`, but PowerShell does not honor `#Requires` inside block comments at parse time, so the script would still hang on `Connect-CrmOnlineDiscovery` under pwsh — exactly the failure R12 is designed to catch. The fix introduces `stripBlockComments()` in `extractor.js`, which length-preservingly space-fills every `<# ... #>` range. The guard regex now tests against this view, so block-comment-nested `#Requires` lexemes are correctly NOT recognized as guards while line-comment `#Requires` directives (the form PS actually honors) ARE recognized.
...
- `probe-R12-block-comment-requires.ps1` (v0.4.1) — true positive: `<# #Requires -Version 5.1 #>` followed by `Connect-CrmOnlineDiscovery`. The block-comment-nested directive is not honored by PS, so R12 fires. Pins the round-2 SHOWSTOPPER fix.
- `probe-R12-block-comment-requires-line-form.ps1` (v0.4.1) — true positive: round-2 reviewer's exact reproduction (`<#` and `#>` on their own lines, `#Requires -Version 5.1` at column 0 on a middle line). R12 fires.
...
- `probe-R28-post-quoted-method-no-guard.ps1` — POST mutation with quoted method literal (`-Method "POST"`), no guard. R28 fires. Pins the v0.4.2 round-1 HIGH-1 fix (`requires_present` uses `normalizedContent`).
...
### R37 — OData `$filter` or `$select` on a Lookup attribute uses bare logicalname (v0.4.4 widening)

The Dataverse Web API represents Lookup attribute values via an `_<logicalname>_value` annotation (e.g., `_appmoduleidunique_value`). This annotation holds the raw GUID of the referenced record. It is the form that must be used in both `$filter` equality expressions and `$select` column lists. The bare logicalname (e.g., `appmoduleidunique`) is a navigation property name for `$expand`; it is not a queryable scalar property.

Using the bare logicalname in `$filter` OR `$select` produces:
...
Correct forms:
- `$filter`: `appmodulecomponents?$filter=_appmoduleidunique_value eq <guid>`
- `$select`: `appmodulecomponents?$select=objectid,_appmoduleidunique_value`
...
- OData Lookup `$select` pattern — "Lookup property data" section: single-valued navigation properties cannot be used in `$select`; use `_<logicalname>_value` instead: https://learn.microsoft.com/en-us/power-apps/developer/data-platform/webapi/query/select-columns

**Implementation (v0.4.4):**

R37 is a `regex-template` rule operating on the `joinPlusContent` content view (v0.4.4 addition). The view collapses PowerShell `+`-based string concatenation across line boundaries into single logical strings before matching. This closes the Gap 2 structural limitation noted in PR #5 review: patterns straddling a `+` concat boundary were invisible to `strippedContent`-based matching.

The pattern has two arms:

1. **`$filter` arm** (`$filter=...<name> eq`): detects bare Lookup logicalnames in filter equality expressions. The `\b` word boundary before the logicalname ensures `_appmoduleidunique_value eq` does NOT match (`_` is `\w`, so no boundary exists before `a`).

2. **`$select` arm** (`appmodulecomponents...$select=...<name>`): detects bare Lookup logicalnames in `$select` column lists. This arm is **EntitySet-anchored** to `appmodulecomponents` to avoid false positives on `appmodule.appmoduleidunique` (Type=Uniqueidentifier on the `appmodule` entity; the same logicalname refers to different types on different entities). Without the anchor, querying `appmodules?$select=appmoduleid,appmoduleidunique,...` would false-positive even though `appmoduleidunique` is a valid Uniqueidentifier column on `appmodule`.

The `joinPlusContent` view (v0.4.4, `src/extractor.js`): collapses adjacent double- or single-quoted string literals joined by `+` across line boundaries by removing the `+` and the newline. Iteration repeats until no more joins are possible (handles chains). Backtick statement-continuations (`` ` `` at EOL without `+` and adjacent quote) are NOT collapsed (different pattern). Variable interpolation inside strings is preserved intact.

The watch list is seeded with `appmoduleidunique`. Extend it via:
...
- `$select=_appmoduleidunique_value` — leading `_` breaks `\b` before `appmoduleidunique`. R37 does not fire.
- `$expand=appmoduleid(...)` uses the navigation property name `appmoduleid` (not `appmoduleidunique`). R37 does not fire.
- `appmodules?$select=appmoduleid,appmoduleidunique,...` — the `$select` arm is anchored to `appmodulecomponents`; the `appmodules` EntitySet does not match. R37 does not fire.
- Multi-line `+` concat with correct `_appmoduleidunique_value` form: after joining, `_` still breaks `\b`. R37 does not fire.
- PowerShell backtick statement-continuation (`` ` `` at EOL, not `+`): `joinPlusContent` does not collapse backtick continuations. R37 does not fire unless the collapsed string contains the bare name.

**Probes (v0.4.4):**

- `probe-R37-P1-bare-filter.ps1` — true positive (v0.4.3): `$filter=appmoduleidunique eq <guid>`. Exact bug. R37 fires.
- `probe-R37-P2-combined-predicate.ps1` — true positive (v0.4.3): combined predicate `componenttype eq 62 and appmoduleidunique eq <guid>`. R37 fires.
- `probe-R37-P3-select-bare-lookup.ps1` — true positive (v0.4.4 Gap 1): `$select=objectid,componenttype,appmoduleidunique` against `appmodulecomponents`. R37 fires.
- `probe-R37-P4-select-middle-position.ps1` — true positive (v0.4.4 Gap 1): bare Lookup name in middle of `$select` list against `appmodulecomponents`. R37 fires.
- `probe-R37-P5-multiline-concat-filter.ps1` — true positive (v0.4.4 Gap 2): `$filter=...appmoduleidunique eq` straddles a `+` line boundary. After `joinPlusContent` joining, R37 fires.
- `probe-R37-P6-multiline-concat-select.ps1` — true positive (v0.4.4 Gap 1 + Gap 2): `$select=...,appmoduleidunique` against `appmodulecomponents` straddles a `+` line boundary. After joining, R37 fires.
...
- `probe-R37-N2-pk-equality.ps1` — true negative: `$filter=appmodulecomponentid eq <guid>`. PK (Uniqueidentifier), not in watch list. R37 does NOT fire.
- `probe-R37-N3-select-not-filter.ps1` — RETIRED (v0.4.4). Was a v0.4.3 negative anchor for `$select` not firing; now fires. File preserved for historical traceability; NOT registered in run-battery.js.
- `probe-R37-N4-expand-not-filter.ps1` — true negative: `$expand=appmoduleid($select=name)`. Navigation prop; `appmoduleidunique` not present. R37 does NOT fire.
- `probe-R37-N5-appmodules-uniqueidentifier.ps1` — true negative (v0.4.4 Gap 1 entity-anchor guard): `appmodules?$select=appmoduleid,appmoduleidunique,...`. `appmodule.appmoduleidunique` is Type=Uniqueidentifier; `$select` arm anchored to `appmodulecomponents` only. R37 does NOT fire.
- `probe-R37-N6-pk-in-select.ps1` — true negative (v0.4.4 Gap 1): `$select=appmodulecomponentid,objectid`. PK not in watch list. R37 does NOT fire.
- `probe-R37-N7-multiline-concat-correct.ps1` — true negative (v0.4.4 Gap 2 regression anchor): multi-line `+` concat with correct `_appmoduleidunique_value` form. After joining, `_` breaks `\b`. R37 does NOT fire.
- `probe-R37-N8-expand-nav-prop.ps1` — true negative: `$expand=appmoduleid(...)`. Navigation prop; watch-list name absent. R37 does NOT fire.
- `probe-R37-N9-backtick-continuation-not-concat.ps1` — true negative (v0.4.4 Gap 2 boundary): backtick statement-continuation; `joinPlusContent` does not collapse it. Correct `_value` form in URL. R37 does NOT fire.

**Known limitations:**

- **Watch-list scope:** R37 only fires for Lookup logicalnames explicitly listed in `rules/registry.json` under R37's `variables` array. Lookup attributes on other entities (e.g., `regardingobjectid`, `ownerid`) are not caught unless added to the list. The intentionally conservative hard-coded approach has zero false positives for unlisted names. Extend as new Lookup filter or select bugs are encountered.
- **`$select` arm EntitySet-scoped:** The `$select` detection is anchored to `appmodulecomponents` to resolve the entity-context ambiguity (`appmoduleidunique` is Uniqueidentifier on `appmodule`, Lookup on `appmodulecomponent`). A `$select` against a different EntitySet that happens to list a Lookup name not in the watch list will not fire. This is an accepted trade-off of the hard-coded-list approach.
- **Pattern matched in `joinPlusContent`:** Full-line `#` comment text is stripped (via `strippedContent` base before joining). Inline comments after code are retained; a trailing inline comment containing the pattern could false-positive, though unlikely for a Dataverse API URL fragment.
...
- Previously (v0.3.1) the test ran against `rawContent`, which matched a block-comment-nested directive and suppressed the rule even though PS would not honor the directive at runtime. Round-2 of the PR #3 review flagged this as a SHOWSTOPPER false negative; v0.4.1 closes the gap.
