...
| extractor-json-error | built-in | ERROR | A here-string payload could not be parsed as JSON; commonly caused by unescaped PowerShell variable interpolation inside a double-quoted here-string (`@"..."@`). v0.5.2: the extractor now skips `JSON.parse` on here-strings whose trimmed body does not start with `{` or `[` (prose/empty bodies are silently skipped); JSON-shaped but malformed bodies and `$()`-interpolated bodies still produce `extractor-json-error`. |
...
### R45 -- Statement keyword inside grouping operator `(...)` (v0.7.0, content-view fix v0.7.2)
...
**Detection strategy (v0.7.2):** Single `regex` rule evaluated against the `strippedNoBlockComments` content view. Pattern: `(?<![$@\w])(\()\s*(if|foreach|for|while|switch|do|trap)\b`. The negative lookbehind `(?<![$@\w])` prevents firing when the `(` is preceded by `$` (subexpression operator `$(...)`), `@` (array subexpression `@(...)`), or a word character (cmdlet name, variable, or identifier immediately before the paren).

`strippedNoBlockComments` is produced by applying `stripBlockComments()` to `rawContent` first (blanking `<# ... #>` bodies while the `#>` close token is still intact), then applying `stripComments()` to the result (blanking full-line `#` comment lines). **Order matters:** `stripComments` matches `^\s*#.*$`, which would erase the `#>` close token if run first -- leaving `stripBlockComments` unable to find the block boundary and thus leaving the body unstripped. Running `stripBlockComments` first on raw content guarantees the close token is found. Both transforms are **length-preserving** (space-fill, newlines kept), so byte-offset alignment for match-index-to-line-number reporting is maintained. String literals are intentionally **not** stripped -- see the documented string-literal false positive below.

**Why `strippedNoBlockComments` and not `strippedContent` (v0.7.2 fix):** Doc-comments and explanatory `<# ... #>` block comments sometimes contain prose such as `WRONG: use (if ...) not $(if ...)`. Under the previous `strippedContent` view, block-comment bodies survived into the evaluation surface and the pattern matched, producing a false positive. Block-comment text is never executable -- per `about_Comments`: "All text within the block is treated as part of the same comment." R45 must not fire on comment prose.

The fix uses the `content_view: "strippedNoBlockComments"` field in the registry entry (v0.7.2 generic dispatch mechanism) rather than hardcoding the view selection for R45 in `validator.js`. Any future rule that needs the same view can declare it in the same way without a code change.

**Substrate citation for the v0.7.2 fix:**
- about_Comments: https://learn.microsoft.com/en-us/powershell/module/microsoft.powershell.core/about/about_comments -- "All text within the block is treated as part of the same comment, including whitespace" -- block-comment body text is inert; keywords inside `<# ... #>` are never executed.
...
- `<# ... (if ...) ... #>` -- keyword inside a block comment; `strippedNoBlockComments` blanks the block body before matching.
- `# ... (if ...)` -- keyword in a full-line `#` comment; `strippedContent` (the base of `strippedNoBlockComments`) already strips line comments.
...
- `probe-R45-string-literal-fp.ps1` -- FIRE (documented false positive): `(if ...)` inside a double-quoted string literal. `strippedNoBlockComments` does not strip string content (intentional; consistent with R33/R34). The rule fires on the string. Authors should use full-line `#` comments for documentation of deprecated usage forms.

**Probes (v0.7.2 block-comment false-positive fix):**

- `probe-R45-block-comment-no-fire.ps1` -- NO FIRE (FP-fix anchor): `(if ...)`, `(foreach ...)`, `(while ...)` in a `<# ... #>` block-comment body (including `.SYNOPSIS`/`.DESCRIPTION` doc-comments). `strippedNoBlockComments` blanks the block body before matching. R45 MUST NOT fire. This is the primary false-positive class fixed in v0.7.2.
- `probe-R45-line-comment-no-fire-regression.ps1` -- NO FIRE (regression anchor): `(if ...)`, `(foreach ...)`, `(while ...)` in full-line `#` comments. The `strippedContent` base of `strippedNoBlockComments` strips line comments; this behavior must survive the v0.7.2 view switch unchanged.
- `probe-R45-real-code-fire-regression.ps1` -- FIRE (regression anchor): real executable `(if (...) { ... } else { ... })` in code, no comment context. The block-comment FP fix must not suppress detection of the actual antipattern.
- `probe-R45-mixed-block-comment-and-real-code.ps1` -- FIRE EXACTLY ONCE (mixed probe): a `<# ... #>` block comment containing `(while ...)` (must not fire) plus a separate real executable `(foreach (...) {...})` line (must fire). Net: R45 fires exactly once. Validates that block-comment blanking is range-bounded and does not bleed into adjacent executable code.
...
- about_Comments: https://learn.microsoft.com/en-us/powershell/module/microsoft.powershell.core/about/about_comments -- block-comment bodies are inert comment text; cite for the v0.7.2 FP fix.

**Known limitations:**

- **String-literal false positive (documented, intentional):** `(if ...)` inside a string literal fires R45 because `strippedNoBlockComments` does not strip string content (intentional). See `probe-R45-string-literal-fp.ps1`. Workaround: put deprecated-usage documentation in full-line `#` comments rather than string literals.
...
### extractor-json-error — here-string body could not be parsed as JSON

PowerShell here-strings (`@"..."@` and `@'...'@`) hold arbitrary literal text — they are not inherently JSON containers. See: https://learn.microsoft.com/en-us/powershell/module/microsoft.powershell.core/about/about_quoting_rules. The extractor uses here-strings as the primary vehicle for Dataverse Web API JSON payloads (e.g., `Invoke-RestMethod -Body $body` where `$body` was assigned from a here-string). Prior to v0.5.2, the extractor attempted `JSON.parse` on every here-string body regardless of content, causing spurious `extractor-json-error` findings on prose here-strings — for example, a module description block like `@" This module provisions the PMO Core tier. "@` would produce a false-positive parse error with no corresponding runtime failure.

**v0.5.2 — JSON-shape guard:** The extractor now inspects the trimmed here-string body before calling `JSON.parse`. Only bodies whose first character is `{` or `[` are treated as JSON and parsed. Prose bodies, empty bodies, and bodies that start with any other character are silently skipped — they were never intended as JSON payloads and `JSON.parse` on them never reflected a real runtime condition. This eliminates the false-positive class entirely.

**v0.7.1 — C# attribute guard:** A `[`-opening body requires an additional check before `JSON.parse` is called. A C# P/Invoke here-string of the form `[DllImport("kernel32.dll", ...)]` trims to a body whose first character is `[`, passes the v0.5.2 shape guard, but is not valid JSON. The guard now requires that the first non-whitespace character after `[` is a JSON array element-start character (`"`, `{`, `[`, digit, `]`, `t`, `f`, or `n` per RFC 8259). Identifier characters like `D` (DllImport), `S` (StructLayout), or `O` (Out) cause the body to be skipped. JSON arrays (e.g., `[{ "id": 1 }]`) still reach `JSON.parse` correctly; the existing regression probe `probe-extractor-json-array.ps1` anchors this. Known limitation: a C# attribute whose name begins with `t`, `f`, or `n` would pass the guard and fail `JSON.parse` — an acceptable one-extra-parse-call edge case, not a false error on a new surface.

**Retained error conditions (regression-guarded):** Two shapes still produce `extractor-json-error` and are intentional:
- **JSON-shaped but malformed bodies:** a here-string whose trimmed body starts with `{` or `[` but is not valid JSON (e.g., contains unescaped `$()`-interpolation residues or truncated structure). These represent real authoring errors where the author intended a JSON payload but produced a broken one. The linter correctly surfaces them.
- **`$()`-interpolated bodies:** double-quoted here-strings containing `$()` subexpressions produce body text with literal `$(...)` fragments that are not valid JSON. These also start with `{` (e.g., `@" { "id": "$(Get-Something)" } "@`), so they remain in the parse path and correctly fire `extractor-json-error`.

**Probes (v0.5.2):**

- `probe-extractor-prose-herestring-no-fire.ps1` — prose here-string body (module description text); `extractor-json-error` must NOT fire. Pins the false-positive fix.
- `probe-extractor-empty-herestring-no-fire.ps1` — empty here-string body; `extractor-json-error` must NOT fire. Regression anchor for edge case.
- `probe-extractor-valid-json-herestring-no-fire.ps1` — well-formed JSON `{ }` body; `extractor-json-error` must NOT fire. Regression anchor confirming the JSON-parse path is preserved.
- `probe-extractor-json-herestring-regression.ps1` — JSON body with `$()` interpolation; `extractor-json-error` MUST fire. Regression anchor confirming interpolated JSON bodies still error.
- `probe-extractor-malformed-json-herestring.ps1` — JSON-shaped but structurally invalid body (`{ "key": INVALID }`); `extractor-json-error` MUST fire. Regression anchor confirming malformed JSON bodies still error.
- `probe-extractor-interpolation-herestring.ps1` — double-quoted here-string with `$(...)` subexpression in a JSON-shaped body; `extractor-json-error` MUST fire. Regression anchor confirming interpolation residues are not silently swallowed.

**Probes (v0.7.1):**

- `probe-extractor-cs-attribute-no-fire.ps1` — C# P/Invoke here-string body `[DllImport("kernel32.dll", ...)]`; `extractor-json-error` must NOT fire. Pins the C# attribute false-positive fix.
- `probe-extractor-json-array-still-parsed.ps1` — valid JSON array `[{ "logicalName": ... }]`; `extractor-json-error` must NOT fire. Regression anchor confirming `[`-opening JSON arrays still parse after the C# attribute guard.

**Substrate citation:**

- about_Quoting_Rules: https://learn.microsoft.com/en-us/powershell/module/microsoft.powershell.core/about/about_quoting_rules — here-strings hold arbitrary literal text, not necessarily JSON. "A here-string is a single-quoted or double-quoted string in which quotation marks are interpreted literally."
